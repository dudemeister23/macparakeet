...
final class DiarizationMetricsTests: XCTestCase {
    ...
    func testDERCountsOverlapAsSpeakerTimeByDefault() {
        let reference = [
            segment("A", 0, 1000),
            segment("B", 500, 1000),
        ]
        ...
        XCTAssertEqual(result.missedMs, 500)
        XCTAssertEqual(result.falseAlarmMs, 0)
        XCTAssertEqual(result.confusionMs, 0)
        XCTAssertEqual(result.totalReferenceMs, 1500)
        XCTAssertEqual(result.der, 1.0 / 3.0, accuracy: 0.0001)
        XCTAssertEqual(result.collarMs, 0)
        XCTAssertFalse(result.skipOverlap)
    }

    func testDERSkipOverlapRemovesReferenceOverlapRegions() {
        let reference = [
            segment("A", 0, 1000),
            segment("B", 500, 1000),
        ]
        ...
        let result = DiarizationMetrics.der(
            reference: reference,
            hypothesis: hypothesis,
            options: DiarizationScoringOptions(collarMs: 0, skipOverlap: true)
        )

        XCTAssertEqual(result.missedMs, 0)
        XCTAssertEqual(result.falseAlarmMs, 0)
        XCTAssertEqual(result.confusionMs, 0)
        XCTAssertEqual(result.totalReferenceMs, 500)
        XCTAssertEqual(result.der, 0, accuracy: 0.0001)
        XCTAssertTrue(result.skipOverlap)
    }

    func testDERCollarExcludesReferenceBoundaryRegions() {
        let reference = [
            segment("A", 0, 1000),
            segment("B", 1000, 2000),
        ]
        let hypothesis = [
            segment("H1", 0, 900),
            segment("H2", 1100, 2000),
        ]

        let withoutCollar = DiarizationMetrics.der(reference: reference, hypothesis: hypothesis)
        let withCollar = DiarizationMetrics.der(
            reference: reference,
            hypothesis: hypothesis,
            options: DiarizationScoringOptions(collarMs: 200)
        )

        XCTAssertEqual(withoutCollar.missedMs, 200)
        XCTAssertEqual(withoutCollar.totalReferenceMs, 2000)
        XCTAssertEqual(withoutCollar.der, 0.1, accuracy: 0.0001)
        XCTAssertEqual(withCollar.missedMs, 0)
        XCTAssertEqual(withCollar.totalReferenceMs, 1600)
        XCTAssertEqual(withCollar.der, 0, accuracy: 0.0001)
        XCTAssertEqual(withCollar.collarMs, 200)
    }

    func testDERDoesNotCountFalseAlarmInsideFullyExcludedCollar() {
        let reference = [
            segment("A", 100, 200),
        ]
        let hypothesis = [
        ...
        let result = DiarizationMetrics.der(
            reference: reference,
            hypothesis: hypothesis,
            options: DiarizationScoringOptions(collarMs: 400)
        )

        XCTAssertEqual(result.missedMs, 0)
        XCTAssertEqual(result.falseAlarmMs, 0)
        XCTAssertEqual(result.confusionMs, 0)
        XCTAssertEqual(result.totalReferenceMs, 0)
        XCTAssertEqual(result.der, 0, accuracy: 0.0001)
    }

    func testCoverageUsesScoringOptions() {
        let reference = [
            segment("A", 0, 1000),
            segment("B", 500, 1000),
        ]
        ...
        let defaultCoverage = DiarizationMetrics.coverage(reference: reference, hypothesis: hypothesis)
        let skipOverlapCoverage = DiarizationMetrics.coverage(
            reference: reference,
            hypothesis: hypothesis,
            options: DiarizationScoringOptions(skipOverlap: true)
        )

        XCTAssertEqual(defaultCoverage, 0.5, accuracy: 0.0001)
        XCTAssertEqual(skipOverlapCoverage, 1.0, accuracy: 0.0001)
    }
